...
class DirectoryRouter {
	...
	constructor(app){
		...
		this.onBeforeCreate(app)
		
		this.context = _initAppContext(app)
		this.imports = _initImports()
		this.settings = _initSettings()
		
		this.onCreate(this.context, this.imports, this.settings)
	...
	_initAppContext(){
		...
		_defaultAppContext = {
			...
			dirs: {
				routes: app.get("routes") || "routes",
				root: path.dirname(require.main.filename),
			}
		}
	
		const app_context = Object.extend({}, _defaultAppContext, context)
		return app_context
	}
